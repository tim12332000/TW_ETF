...
def get_optimal_leverage():
    ...
    df = df.dropna()
    vt_returns = df['VT_Price'].pct_change().fillna(0)
    ...
    for lev in leverages:
        ...
        # 若 lev <= 1，假設閒置現金無利息(0%)且無成本
        annual_cost = max(0, lev - 1) * 0.015
        daily_cost = annual_cost / 252
        
        lev_returns = lev * vt_returns - daily_cost
        
        # 避免破產導致的錯誤 (如果單日跌幅超過 100/lev)
        # 用 log returns 或直接 cumprod, clipping at -1
        lev_returns = np.clip(lev_returns, -1.0, None)
    ...
    fig.tight_layout()  
    plt.title('VT 不同槓桿倍數之「CAGR 報酬」與「最大回撤」關係圖 (尋找數學最優解)', fontsize=16)
    ...
    output_file = 'vt_optimal_leverage.png'
