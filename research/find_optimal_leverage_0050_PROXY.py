...
def get_optimal_leverage():
    print("正在下載資料 (EWT，作為台灣 0050 的長期代理)...")
    # EWT (iShares MSCI Taiwan ETF) 於 2000 年成立，可作為台灣大型權值股的代理，能涵蓋更完整的週期
    data = yf.download('EWT', progress=False)
    
    df = pd.DataFrame()
    if isinstance(data.columns, pd.MultiIndex):
        df['Price'] = data['Close']['EWT']
    else:
        df['Price'] = data['Close']
    ...
    fig.tight_layout()  
    plt.title(f'台灣 0050 長期代理 EWT (2000-至今) 尋找數學最優解', fontsize=16)
    ...
    output_file = 'tw_0050_optimal_leverage_ewt.png'
    plt.savefig(output_file, dpi=150)
    
    print("\n" + "=" * 50)
    print("台灣市值前 50 大 (2000 至今回測)")
    print("=" * 50)
